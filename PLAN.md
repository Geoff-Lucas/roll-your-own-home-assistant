...
- **Done (Sept 2026): typed-in recipes, and editing any recipe.** "✏️ Add a recipe" on the Recipes tab (`frontend/src/lib/recipes/RecipeEditor.svelte`); ingredients and steps one per line. The server tidies what people type (blank lines, bullets, step numbers — `routers/recipes.py`) and parses ingredients with the same `parse_ingredient_line` the importer uses, so a typed recipe is indistinguishable from an imported one for the meal plan and a future shopping list. The same editor edits imported recipes too (to fix a bad scrape) and deletes (two taps). No photo for typed recipes yet: there's no camera, and upload would want the phone app.
...
- **Hardware:** a USB microphone is attached and works. Speaker output is the monitor's HDMI speakers, played through PulseAudio (`HOME_ORGANIZER_AUDIO_DEVICE=pulse`) — a raw ALSA device gets locked out for several seconds whenever the screen wakes from sleep, see `deploy/README.md` "Audio". All sound goes through `app/audio/player.py` so timer chimes and spoken replies share one path.
- **Build order for voice:** commands are plain functions usable by tap first, voice second. Done: **timers, alarms, stopwatch** (`app/timers/`, server-side so alarms ring regardless of what the screen is showing). **Also done: the tap-to-talk loop** (`app/voice/`): mic (`arecord`, energy-based end-of-speech) → local faster-whisper → rule-based commands (timers/alarms/stopwatch, time/date, weather) → espeak-ng or Piper, with the reply also on screen. Commands are plain functions of (text, context), tested with a fake clock and canned weather. **Also done: the "Hey Jarvis" wake word** (`app/voice/wakeword.py`, openWakeWord 0.4.0 bundled model, opt-in via HOME_ORGANIZER_VOICE_WAKEWORD_ENABLED; the listener releases the mic for the conversation and resumes after the reply). **Also done: the Claude API fallback** (`app/voice/skills/claude_fallback.py`, off by default via HOME_ORGANIZER_VOICE_CLAUDE_ENABLED + an API key; the last skill tried, for open-ended questions nothing rule-based understands — one-shot only, no conversation memory yet). Not yet: calendar-read/meal-plan/conversion commands, follow-up questions (the conversation keeping the mic open for a continuation after a Claude answer), calendar adds by voice (always read back for confirmation), recipe search, cook-mode, shopping list. Further out: letting Claude *take* actions (create events, add meals) via tool-calling against the existing service-layer functions — still no MCP server needed for that, since it's all in-process; an MCP server would only matter if an *external* Claude client (this session, Claude Desktop) should be able to reach into the home organizer directly, which is a separate, later idea.
